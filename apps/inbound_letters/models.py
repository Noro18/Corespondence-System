...
class InboundLetter(models.Model):
    ...
    thumbnail = models.ImageField(upload_to="inbound_thumbnails/", blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not is_new:
            try:
                old_pdf = InboundLetter.objects.filter(pk=self.pk).values_list('pdf_file', flat=True).first()
            except Exception:
                pass
